...
- 実際の provider execution を導入する前に、provider と Agent panel の disconnected/error shell state とローカル recovery guidance を追加する。
