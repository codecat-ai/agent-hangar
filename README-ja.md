...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化します。Tauri/React shell には provider 概要と Agent 状態パネルの足場があります。
...
- provider 概要、provider health/capability summary、Agent 状態パネルを持つ React/Tauri shell の足場。
...
- provider profile 編集と model discovery refresh 状態をデスクトップの provider-management flow に接続する。
