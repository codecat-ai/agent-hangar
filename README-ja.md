...
- schema version 付きの delegation、review、broadcast、escalation item、型付き validation issue、未解決および高優先度の sort、secret-safe な body、note、audit detail summary、next-action hint、確定的な Markdown preview data を扱う純粋な collaboration inbox と audit-history helper。
- execution graph panel 内の React collaboration inbox と audit-history preview 領域。accessible region、未解決 count、sanitize 済み recent history、next-action hint、copy 可能な Markdown を表示します。
...
現在の demo graph、ローカル execution trail、collaboration inbox、audit-history preview、guarded execution controls、run evidence export preview は、実際の provider call、network call、shell command、secret、顧客データなしで、planner、researcher、reviewer がタスクを作成、計画、割り当て、引き継ぎ、レビュー、完了、安全なローカル制御状態遷移、運用者の次アクション判断まで進める様子を示します。
...
- Collaboration inbox と audit-history preview は、表示や copy の前に body、note、recent history、Markdown text を sanitize し、生の API key、encrypted key material、token、顧客データ、provider secret を公開しません。
...
- 実装済みの delegation、review、broadcast、escalation、audit history を中心にローカル demo の協調 view を強化しつつ、execution controls は確定的で provider-free のまま維持する。
