...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化し、role preset、binding、version history、workspace-aware validation、有向 handoff/dependency edge、runnable-node summary、import/export 向け validation report を備えた local-first prompt template と execution graph の足場も含みます。Tauri/React shell には、ローカル provider profile 管理、template studio の基盤パネル、execution graph preview、Agent 状態パネルがあります。
...
- graph count、issue summary、next runnable node を表示する React execution graph preview。
...
- execution graph の足場の上に、確定的なローカル demo execution trail を作る。
...
- 依存関係をインストールすると、フロントエンドテストで provider profile、prompt template、execution graph の足場、React shell の振る舞いを検証できます。

現在の demo graph は、実際の provider call を行わず planner から reviewer への handoff readiness を示します。将来の demo workspace では、planner、researcher、implementer、reviewer Agent が型付きの引き継ぎと確定的なローカルイベントを通じて長時間タスクに協調する様子を示す予定です。
