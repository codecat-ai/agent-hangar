...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化し、role preset、binding、version history、workspace-aware validation、有向 handoff/dependency edge、runnable-node summary、timeline replay、audit entry、型付き collaboration issue、import/export 向け validation report を備えた local-first prompt template、execution graph、確定的な demo execution trail、schema version 付き demo workspace seed、ローカル run evidence export、保護されたローカル execution-control、collaboration inbox 正規化、collaboration acknowledge/resolve のローカル永続化、audit-history preview helper の足場も含みます。Tauri/React shell には、ローカル provider profile 管理、template studio の基盤パネル、ローカル trail count 付きの execution graph preview、planner/researcher/implementer/reviewer 協調の compact demo workspace summary、ローカルの pause/resume/cancel/retry 状態遷移を扱う guarded execution controls、acknowledge/resolve action と persistence fallback を備えた collaboration inbox、audit-history preview の領域、ローカル run evidence export preview/copy action、Agent 状態パネルがあります。
...
- planner、researcher、implementer、reviewer の協調を clone-safe に表す、schema version 付き graph、trail、collaboration、audit preview 入力を生成する純粋で確定的な demo workspace seed helper。
...
- schema version 付きの delegation、review、broadcast、escalation item、型付き validation issue と mutation issue、未解決および高優先度の sort、clone-safe な acknowledge/resolve 遷移、secret-safe な body、note、reason、audit detail summary、persistence payload、next-action hint、確定的な Markdown preview data を扱う純粋な collaboration inbox と audit-history helper。
- execution graph panel 内の React collaboration inbox、audit-history preview、compact demo workspace summary 領域。accessible region、role count、collaboration mix、未解決 count、acknowledge/resolve button、localStorage persistence fallback、sanitize 済み recent history、next-action hint、copy 可能な Markdown を表示します。
...
- provider profile 管理、prompt template 管理、provider health/capability summary、Agent 状態パネルを持つ React/Tauri shell の足場。
...
- 依存関係をインストールすると、フロントエンドテストで provider profile、prompt template、execution graph と trail の足場、React shell の振る舞いを検証できます。

現在の demo graph、ローカル execution trail、collaboration inbox、audit-history preview、guarded execution controls、run evidence export preview は、実際の provider call、network call、shell command、secret、顧客データなしで、planner、researcher、implementer、reviewer がタスクを作成、計画、割り当て、委任、実装、引き継ぎ、レビュー、完了、安全なローカル制御状態遷移、collaboration acknowledge/resolve、broadcast、escalation、運用者の次アクション判断まで進める様子を示します。
...
- Collaboration inbox mutation と audit-history preview は、表示、copy、永続化 JSON への書き込みの前に body、note、reason、recent history、Markdown text を sanitize し、生の API key、encrypted key material、token、顧客データ、provider secret を公開しません。
...
- 拡張済みのローカル demo workspace を使い、collaboration triage、audit history、compact operator summary を強化しつつ、execution controls は確定的で provider-free のまま維持する。
