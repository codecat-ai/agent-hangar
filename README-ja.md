...
- walkthrough、scenario bundle、workspace portability manifest、import/export dry run、collaboration triage/audit preview、run evidence、onboarding guidance の copied Markdown と構造化 preview data を比較する、純粋で確定的な source-checkout evidence quality-gate helper。missing Markdown、unsupported/missing schema、source-checkout 以外の mode、install command text、secret らしい content、実用的に確認できる count mismatch を検出し、compact な status/severity count、issue、next action、copy-ready Markdown を provider、network、shell、registry、Tauri call なしで返します。
...
- Source-checkout evidence quality-gate preview は、可能な範囲でローカルの構造化 count と copied Markdown を比較し、missing Markdown、unsupported schema、source-checkout 以外の mode、install command text、secret らしい content を拒否し、copy 前に issue、next action、Markdown を再 sanitize します。
...
- provider call を有効にしたり desktop package release を準備したりする前に、secure storage、cancellation、cache replacement、retry policy、live-adapter test を中心に live discovery readiness review を完了する。
