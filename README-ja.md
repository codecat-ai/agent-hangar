...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化し、role preset、変数、version history、validation を備えた local-first prompt template harness も含みます。Tauri/React shell には、ローカル provider profile 管理、template studio の基盤パネル、Agent 状態パネルがあります。
...
- 確定的な role preset、template の作成/更新/削除、`{{variableName}}` の変数抽出、不変の version history、provider/model/escalation validation、policy binding check を扱う純粋な prompt template helper。
- preset/template の閲覧、role preset からの作成、prompt record の編集、provider secret を出さない validation/version status 表示を行う React template studio 基盤。
...
- policy/tool binding と workspace import/export の準備に向けて template validation を深める。
...
- policy binding、tool requirement、将来の workspace portability に向けて Agent template studio の validation を深める。
- provider と template の基盤が安定した後に execution graph の足場を追加する。
