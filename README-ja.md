...
- execution graph panel 内の React scenario selector、collaboration inbox、audit-history preview、scenario evidence bundle preview、workspace portability manifest preview、compact demo workspace summary 領域。accessible control と region、role count、collaboration mix、未解決 count、status/priority/type/search controls、acknowledge/resolve button、localStorage persistence fallback、sanitize 済み recent history、next-action hint、source-checkout portability blocker、copy 可能な Markdown を表示します。
...
現在のローカル demo scenario、execution trail、collaboration inbox、audit-history preview、guarded execution controls、run evidence export preview、scenario evidence bundle preview、workspace portability manifest preview は、planner、researcher、implementer、reviewer が成功する coordination path と blocked/failure-recovery path の両方で動く様子を示します。実際の provider call、network call、shell command、secret、顧客データなしで、作成、計画、割り当て、委任、実装、引き継ぎ、レビュー、完了、failed と blocked 状態、安全なローカル制御状態遷移、collaboration acknowledge/resolve、broadcast、escalation、運用者の次アクション判断、source-checkout portability blocker を扱います。
