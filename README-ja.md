...
- execution graph panel 内の React collaboration inbox、audit-history preview、compact demo workspace summary 領域。accessible region、role count、collaboration mix、未解決 count、status/priority/type/search controls、acknowledge/resolve button、localStorage persistence fallback、sanitize 済み recent history、next-action hint、copy 可能な Markdown を表示します。
...
- workspace portability と追加の blocked/failed demo scenario を拡張しつつ、collaboration triage、audit history、compact operator summary、execution controls は確定的で provider-free のまま維持する。
