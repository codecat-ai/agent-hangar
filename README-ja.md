...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化し、role preset、binding、version history、workspace-aware validation、有向 handoff/dependency edge、runnable-node summary、timeline replay、import/export 向け validation report を備えた local-first prompt template、execution graph、確定的な demo execution trail、ローカル run evidence export の足場も含みます。Tauri/React shell には、ローカル provider profile 管理、template studio の基盤パネル、ローカル trail count 付きの execution graph preview、ローカル run evidence export preview/copy action、Agent 状態パネルがあります。
...
現在の demo graph、ローカル execution trail、run evidence export preview は、実際の provider call、network call、shell command、secret、顧客データなしで、planner、researcher、reviewer がタスクを作成、計画、割り当て、引き継ぎ、レビュー、完了まで進める様子を示します。
...
- ローカル demo を run evidence export preview から、pause、resume、cancel、retry、durable audit log のための保護された operator control へ拡張する。
