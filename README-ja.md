...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、暗号化されたローカル provider profile helper、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。frontend harness では、運用者向け provider card のために provider health summary と model capability tag も正規化します。Tauri/React shell には、ローカル provider profile 管理パネルと Agent 状態パネルがあります。
...
- provider profile 管理、provider health/capability summary、Agent 状態パネルを持つ React/Tauri shell の足場。
...
- prompt CRUD、変数、version history、provider/model validation を備えた Agent template studio を拡張する。
