...
Agent Hangar は **active-development** の基盤開発段階です。Rust core harness では、provider 正規化、secret-safe provider card、Agent テンプレート、状態遷移、Agent 間の型付きメッセージルーティングのテストが通っています。Tauri/React shell には provider 概要と Agent 状態パネルの足場があります。
...
現時点では本番用 provider 設定は不要です。予定している設定は local-first かつ secret-safe です。

- Provider profile はローカルに保存します。
- シークレットは debug 文字列、エクスポートされた card、ログ、UI snapshot に出してはいけません。
- Agent が provider capability を使う前に、metadata を正規化します。
...
- 暗号化されたローカル provider profile を追加する。
