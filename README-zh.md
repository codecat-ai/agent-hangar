...
- 纯确定性的 source-checkout onboarding/accessibility helper，提供带 schema version 的 keyboard order、named-region expectation、status-region expectation、setup note、primary walkthrough guidance、blocker/next-action summary 和 secret-safe Markdown，并且不调用 provider、network、shell、registry 或 Tauri。
- 纯确定性的 source-checkout evidence quality-gate helper，会比较 walkthrough、scenario bundle、workspace portability manifest、import/export dry run、collaboration triage/audit preview、run evidence 和 onboarding guidance 中的 copied Markdown 与结构化 preview data；标记缺失 Markdown、不支持或缺失 schema、非 source-checkout mode、安装命令文本、类似 secret 的内容，以及可实际检查的 count mismatch；并返回紧凑 status/severity count、issue、next action 和可复制 Markdown，且不调用 provider、network、shell、registry 或 Tauri。
...
- React execution graph panel 内的 scenario selector、source-checkout onboarding、collaboration inbox、audit-history preview、scenario evidence bundle preview、source-checkout operator walkthrough、source-checkout evidence quality gate、workspace portability manifest preview、workspace import/export dry-run preview 和紧凑 demo workspace summary 区域，包含 accessible control 和 region、角色计数、协作类型分布、未解决计数、status/priority/type/search 控件、acknowledge/resolve 按钮、localStorage persistence fallback、已清理的 recent history、next-action hint、source-checkout setup note、evidence link、evidence quality issue、source-checkout portability blocker、no-mutation dry-run note 和可复制 Markdown。
...
- Source-checkout onboarding preview 会把 guided walkthrough 作为主要 review path，同时保留 provider、template、execution、collaboration 和 portability evidence link；setup note 保持 source-checkout-only，并在渲染或复制前清理文本。
- Source-checkout evidence quality-gate preview 会在可行处比较本地结构化 count 与 copied Markdown，拒绝缺失 Markdown、不支持 schema、非 source-checkout mode、安装命令文本和类似 secret 的内容，并在复制前重新清理 issue、next action 与 Markdown。
...
- 保持 guided source-checkout walkthrough 作为主要 review path，同时保留可见的 provider/template/execution/collaboration/portability evidence。
...
- 在启用 provider call 或准备桌面打包 release 前，先围绕 secure storage、cancellation、cache replacement、retry policy 和 live-adapter test 完成 live discovery readiness review。
