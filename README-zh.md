...
Agent Hangar 处于 **active-development** 基础建设阶段。Rust core harness 目前已有 provider 标准化、secret-safe provider card、加密本地 provider profile helper、Agent 模板、状态转换和 Agent 间类型化消息路由的通过测试。前端 harness 也会为面向运营者的 provider card 标准化 provider health summary 和模型 capability tag，并且现在包含 local-first prompt template、execution graph、确定性 demo execution trail、带 schema version 的 demo workspace seed、本地 run evidence export、受保护的本地 execution-control 脚手架、collaboration inbox 标准化、collaboration acknowledge/resolve 本地持久化和 audit-history preview helper，支持角色 preset、binding、版本历史、workspace-aware 校验、有向 handoff/dependency edge、runnable-node summary、timeline replay、audit entry、类型化 collaboration issue 和面向 import/export 的 validation report。Tauri/React shell 现在包含本地 provider profile 管理、template studio 基础面板、带本地 trail count 的 execution graph preview、用于 planner/researcher/implementer/reviewer 协调的紧凑 demo workspace summary、用于本地 pause/resume/cancel/retry 状态转换的 guarded execution controls、带 acknowledge/resolve action 与本地 persistence fallback 的 collaboration inbox、audit-history preview 区域、本地 run evidence export preview/copy action 和 Agent 状态面板。
...
- 纯确定性 demo workspace seed helper，用 clone-safe 的方式生成 planner、researcher、implementer、reviewer 协调数据，并包含带 schema version 的 graph、trail、collaboration 和 audit preview 输入。
...
- 纯 collaboration inbox 与 audit-history helper，支持带 schema version 的 delegation、review、broadcast、escalation item，类型化 validation 和 mutation issue，未解决和高优先级排序，clone-safe acknowledge/resolve 转换，secret-safe 的 body、note、reason、audit detail summary，persistence payload，next-action hint，以及确定性的 Markdown preview data。
- React execution graph panel 内的 collaboration inbox、audit-history preview 和紧凑 demo workspace summary 区域，包含 accessible region、角色计数、协作类型分布、未解决计数、acknowledge/resolve 按钮、localStorage persistence fallback、已清理的 recent history、next-action hint 和可复制 Markdown。
...
当前 demo graph、本地 execution trail、collaboration inbox、audit-history preview、guarded execution controls 和 run evidence export preview 展示 planner、researcher、implementer、reviewer 如何在没有真实 provider call、network call、shell command、secret 或客户数据的情况下，推动任务完成创建、规划、分配、委派、实现、交接、评审、完成、安全的本地控制状态转换、collaboration acknowledge/resolve、broadcast、escalation 和运营者后续决策。
...
- Collaboration inbox mutation 与 audit-history preview 在渲染、复制或写入持久化 JSON 前会清理 body、note、reason、recent history 和 Markdown 文本，并且不会暴露原始 API key、encrypted key material、token、客户数据或 provider secret。
...
- 使用已扩展的本地 demo workspace 继续强化 collaboration triage、audit history 和紧凑 operator summary，同时保持 execution controls 的确定性和 provider-free 约束。
