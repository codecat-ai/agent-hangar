...
Agent Hangar 处于 **active-development** 基础建设阶段。Rust core harness 目前已有 provider 标准化、secret-safe provider card、加密本地 provider profile helper、Agent 模板、状态转换和 Agent 间类型化消息路由的通过测试。前端 harness 也会为面向运营者的 provider card 标准化 provider health summary 和模型 capability tag，并且现在包含 local-first prompt template、execution graph、确定性 demo execution trail、本地 run evidence export 和受保护的本地 execution-control 脚手架，支持角色 preset、binding、版本历史、workspace-aware 校验、有向 handoff/dependency edge、runnable-node summary、timeline replay、audit entry 和面向 import/export 的 validation report。Tauri/React shell 现在包含本地 provider profile 管理、template studio 基础面板、带本地 trail count 的 execution graph preview、用于本地 pause/resume/cancel/retry 状态转换的 guarded execution controls、本地 run evidence export preview/copy action 和 Agent 状态面板。
...
当前 demo graph、本地 execution trail、guarded execution controls 和 run evidence export preview 展示 planner、researcher、reviewer 如何在没有真实 provider call、network call、shell command、secret 或客户数据的情况下，推动任务完成创建、规划、分配、交接、评审、完成和安全的本地控制状态转换。
...
- 围绕 delegation、review、broadcast、escalation 和 audit history 扩展本地 demo 协作视图，同时保持 execution controls 的确定性和 provider-free 约束。
