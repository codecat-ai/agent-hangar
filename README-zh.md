...
- 在未来任何 live adapter prototype 满足已评审 contract gate 前，让确定性的 provider discovery dry-run preview 和禁用的 fixture-backed adapter shell 保持 local/demo-only。
...
- 默认禁用的 provider discovery adapter shell 已通过 fixture-backed consent、audit 和 redaction 测试实现后，决定是否开展 cadence/completion review。
